...
|---|---|---|---|
| `vite.config.ts` / `rspack.config.ts` / `rsbuild.config.ts` | Bundler config host (`rspfxVite()` / `RspfxPlugin` / `rspfxRsbuild()`). Synthesized from manifests if missing. | No | `rspfx new` / `rspfx migrate` |
