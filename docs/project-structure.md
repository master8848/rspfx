...
```
my-app/
├── rspack.config.ts              # optional — generated by rspfx new / migrate; omit for zero-config (synthesized from manifests)
...
|---|---|---|---|---|
| `rspack.config.ts` | Project config host — `RspfxPlugin` instance (`name`, `framework`, `spfxVersion`, `dev`, `build`, `paths`, `teams`). Loaded via `jiti` when present; when absent the CLI synthesizes the same options from `config/*` + `package.json`. | No — optional (zero-config also works) | Scaffolded (`packages/templates/src/index.ts:88` `rspackConfig()`) or by `rspfx migrate` | — (rspfx-owned; synthesis uses same manifests as Heft/Gulp) |
| `vite.config.ts` | Alternative host — `rspfxVite()` plugin. `rspfx build` spawns one `vite build` per bundle. | Alternative — optional | On Vite projects / `rspfx migrate --bundler vite` only | https://vitejs.dev/guide/ |
