...
Run `rspfx new` to scaffold a minimal project (web part, React, SPFx 1.23 default from `packages/core/src/versions.ts:13` `SPFX_DEFAULT_TARGET`) — use `--yes` for non-interactive runs. The same steps apply for extensions (`--component applicationcustomizer|fieldcustomizer|listviewcommandset|formcustomizer`) and libraries (`--component library`) — each produces `Extension_<id>.xml` / `Library_<id>.xml` via `packages/sppkg-builder/src/xml.ts:181` and was installed in the 2026-08-22 tenant gate.
...
| `.sppkg` zip entries exact | Unzip and list entries; they must equal the `zipEntries` assertion in `packages/sppkg-builder/tests/official-parity.test.ts:1` and `packages/sppkg-builder/tests/sppkg-builder.test.ts:77` (`[Content_Types].xml`, `_rels/.rels` → `/AppManifest.xml`, `AppManifest.xml`, `_rels/AppManifest.xml.rels`, `feature_<id>.xml`, `feature_<id>.xml.config.xml`, `_rels/feature_<id>.xml.rels`, `<featureId>/WebPart_<componentId>.xml` / `<featureId>/Extension_<id>.xml` / `<featureId>/Library_<id>.xml` (`Type="Library"` single-quoted, no `Module`/`Location`/`Instance` via `packages/sppkg-builder/src/xml.ts:181`), `ClientSideAssets.xml`, `ClientSideAssets.xml.config.xml`, `_rels/ClientSideAssets.xml.rels`, `ClientSideAssets/` when `includeClientSideAssets`) with `ProductID` raw GUID, `IsDomainIsolated` `false` included, `DeveloperProperties` 5 keys, `CategoryID`, `Screenshots`, `AppPartConfig` `randomUUID` — see [docs/compatibility.md](compatibility.md) and `reference/FORMATS.md` §4; gate `apps/playground` validates `IsValidAppPackage:true` `Title` `rspfx-playground-client-side-solution` `AppProductID` `22222222-2222-4222-8222-222222222200` |
