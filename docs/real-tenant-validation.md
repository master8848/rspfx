...
| `.sppkg` zip entries exact | Unzip and list entries; they must equal the `zipEntries` assertion in `packages/sppkg-builder/tests/official-parity.test.ts:1` and `packages/sppkg-builder/tests/sppkg-builder.test.ts:77` (`[Content_Types].xml`, `_rels/.rels` → `/AppManifest.xml`, `AppManifest.xml`, `_rels/AppManifest.xml.rels`, `feature_<id>.xml`, `feature_<id>.xml.config.xml`, `_rels/feature_<id>.xml.rels`, `<featureId>/WebPart_<componentId>.xml`, `ClientSideAssets.xml`, `ClientSideAssets.xml.config.xml`, `_rels/ClientSideAssets.xml.rels`, `ClientSideAssets/` when `includeClientSideAssets`) with `ProductID` raw GUID, `IsDomainIsolated` `false` included, `DeveloperProperties` 5 keys, `CategoryID`, `Screenshots`, `AppPartConfig` `randomUUID` — see [docs/compatibility.md](compatibility.md) and `reference/FORMATS.md` §4; gate `apps/playground` validates `IsValidAppPackage:true` `Title` `rspfx-playground-client-side-solution` `AppProductID` `22222222-2222-4222-8222-222222222200` |
