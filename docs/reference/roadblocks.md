...
Gate: scaffold → `rspfx package` → upload `.sppkg` to Microsoft 365 app catalog → install → render in workbench — passed 2026-08-22 for web parts, extensions, and libraries (see [real-tenant-validation.md](real-tenant-validation.md)). Correctness rests on parity checks plus the gate. Remaining is automated CI matrix across SPFx 1.20–1.24 (needs `RSPFX_ACCESS_TOKEN` + `RSPFX_APP_CATALOG_URL` per [commands.md#environment-variables](commands.md#environment-variables)).
...
| No long-term support / no build-plugin ecosystem | Scope to web-parts on SPFx 1.20–1.24, port CI via `rspfx doctor` + `rspfx build` |
...
Tenant CI matrix and official-toolchain bench are hardening tracked in [roadmap.md](roadmap.md); web parts, extensions, and libraries reflect the verified gate per [real-tenant-validation.md](real-tenant-validation.md).
