...
- `rspfx new <name>` — interactive prompts (framework, language, spfx target, pm); flags `--framework <id> --language <ts|js> --spfx-version <v> --pm <pnpm|npm|yarn|bun> --no-install --yes` for non-interactive. Then scaffold + `git init` (no dependency install — run `pnpm|npm|yarn|bun install` yourself).
