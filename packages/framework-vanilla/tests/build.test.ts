...
const FIXTURE = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fixtures', 'vanilla-app');
const OUT_DIR = 'dist';
...
describe('framework-vanilla build', () => {
  beforeAll(() => {
    fs.rmSync(path.join(FIXTURE, OUT_DIR), { recursive: true, force: true });
  });

  it('compiles the vanilla entry with the preset contributions', async () => {
  ...
  it('bundles the component code into test.js', () => {
    const bundle = fs.readFileSync(path.join(FIXTURE, OUT_DIR, 'test.js'), 'utf8');
    expect(bundle).toContain('hello vanilla');
  });
});
