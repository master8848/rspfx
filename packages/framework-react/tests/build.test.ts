...
const FIXTURE = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fixtures', 'react-app');
const OUT_DIR = 'dist';
...
describe('framework-react build', () => {
  beforeAll(() => {
  ...
  it('enables swc react development mode and the refresh plugin when fastRefresh is on', () => {
    ...
    expect(plugin.constructor.name).toBe('ReactRefreshRspackPlugin');
  });
});
