...
import { PreactWebPart } from '../src/webpart.js';

class TestPreactWebPart extends PreactWebPart<{ title: string }> {
  protected renderComponent(props: { title: string }) {
    return h('div', null, props.title);
  }
}

function initialize(webPart: TestPreactWebPart, domElement: HTMLElement): void {
...
describe('PreactWebPart', () => {
  it('renders the preact component into the web part domElement', () => {
    const domElement = document.createElement('div');
    const webPart = new TestPreactWebPart();
